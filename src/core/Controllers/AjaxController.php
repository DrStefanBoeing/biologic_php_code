...
{
    public static function getGeneList(Request $request)
    {
        $primary_gene_symbol = setting('primary_gene_symbol');

        $gene_list = QB::table(setting('rnaseq_db_table'))->distinct()->select($primary_gene_symbol)->get()
            ->pluck($primary_gene_symbol)->flatten();
