...
if ($x_axis != 'one_dimensional')
   {$result_bg = "["; 
    while ($r = mysqli_fetch_array($bg_query)) #Get black background data
	   {extract($r);
	    
		if ($ptm_pos_column == '' | $ptm_pos_column == 'NA')
		   {$name = ${$point_name_column};
			} else 
			   {$name = ${$point_name_column}."-".${$ptm_pos_column}; 
				}; 
				
	    $result_bg = $result_bg."{x:".${$x_axis}.", y:".${$y_axis}.", name:'".$name."'},"; 
	    };
    $result_bg = $result_bg."{x:".${$x_axis}.", y:".${$y_axis}.", name:'".$name."'}]";
    $json_bg = $result_bg;
    ...
    $nrow_hl_query = mysqli_num_rows($highlight_query);
	...
	if ($nrow_hl_query > 0)
	   {
	$result_gene = "["; 
    while ($r = mysqli_fetch_array($highlight_query)) 
       {extract($r);
	    if ($ptm_pos_column == 'NA')
		   {$name = ${$point_name_column};
			} else 
			   {$name = ${$point_name_column}." ".${$ptm_pos_column}; 
				}; 
		 
	    $result_gene = $result_gene."{x:".${$x_axis}.", y:".${$y_axis}.", name:'".$name."'},"; 
	    }
    $result_gene = $result_gene."{x:".${$x_axis}.", y:".${$y_axis}.", name:'".$name."'}]";
    $json_gene = $result_gene;
    };  
   } 
...
if ($x_axis == 'one_dimensional')
   {$result_bg = "[";
    $counter = 1;  
    while ($r = mysqli_fetch_array($base_sql_query)) #Get black background data
	   {extract($r);
		if ($ptm_pos_colun == 'NA')
		   {$name = ${$point_name_column};
			} else 
	    ...
	    $result_bg = $result_bg."{x:".$counter.", y:".${$y_axis}.", name:'".$name."'},"; 
	    };
    $result_bg = $result_bg."{x:".$counter.", y:".${$y_axis}.", name:'".$name."'}]";
    $json_bg = $result_bg;
	  
    #Find data to highlight
    //currently not supported in one dimensional plots
    $result_gene = "["; 
    $result_gene = $result_gene."{x:"."".", y:"."".", name:'".""."'}]";
    $json_gene = $result_gene;
    };  
